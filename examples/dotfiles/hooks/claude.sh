#!/usr/bin/env bash
# hook: claude — clone the statusline repo beside the dotfiles repo, run install.
# Data: repo=<git url>  → $BOTU_repo
_claude_dir() { printf '%s' "$(cd "$BOTU_CONFIG/.." && pwd)/claude-statusline"; }
_claude_apply() {
...
_claude_verify() {
  _hdr "claude-statusline"
  if [[ -x "$HOME/.local/bin/claude-statusline" ]]; then _ok "statusline on PATH"; else _warn "statusline missing — botu apply --only=claude"; fi
}
_claude_fix() { _claude_apply; }
