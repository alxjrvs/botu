...
the config *was* bash (the engine `source`d the `botufile`). That premise was
deliberately overturned: botu was rewritten end-to-end in **TypeScript on Bun**,
shipped as a single self-contained binary, and the config became typed,
validated **TOML** (`botufile.toml`). The ~62 MB compiled-binary floor is the
...
`botu rollback` replays the journal in reverse; `apply --resume` skips done steps;
...
`apply`/`verify`/`fix` that receive a typed `HookApi`. Built-in resources
...
`botufile.<os|host|profile>.toml` merge onto the base; `--profile` (repeatable)
activates named profiles, os/host auto-match. (`src/config/profile.ts`.)

## What stayed true

"One model, two surfaces" survived the rewrite intact: `apply`/`verify`/`fix`/
