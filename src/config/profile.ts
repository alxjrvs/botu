...
// os + host auto-match the machine (overridable via BOTU_OS / BOTU_HOST, which also
...
export function profileContext(env: Env, explicit: readonly string[]): ProfileContext {
  return { os: detectOs(env), host: env.BOTU_HOST ?? hostname(), profiles: new Set(explicit) };
}
...
// Overlay file basenames sourced (if present) after the base botufile.toml, in order.
export function overlayFiles(pc: ProfileContext): string[] {
  const names = [`botufile.${pc.os}.toml`, `botufile.${pc.host}.toml`];
  for (const p of pc.profiles) names.push(`botufile.${p}.toml`);
  return names;
}
